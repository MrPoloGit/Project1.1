...
public void UpdatePlayer(Player player)
{
    ...
    ModPlayer.debugText = smartCursor ? "yes" : "no";
}
      ...
      ref int fY)
{
    ...
    bool flag1 = providedInfo.item.type != 523;
    for (int index1 = reachableStartX; index1 <= reachableEndX; ++index1)
    {
        for (int index2 = reachableStartY; index2 <= reachableEndY; ++index2)
        {
            ...
            if (!tile1.active || TileID.Sets.BreakableWhenPlacing[(int)tile1.type] || Main.tileCut[(int)tile1.type] && tile1.type != (ushort)82 && tile1.type != (ushort)83)
            {
                ...
                if (!flag2 && (!flag1 || tile1.liquid <= (byte)0) && (tile1.wall > (ushort)0 || tile2.active && ((int)tile2.slope() % 2 != 1) && (Main.tileSolid[(int)tile2.type] && !Main.tileNoAttach[(int)tile2.type] && (!Main.tileSolidTop[(int)tile2.type] && !TileID.Sets.NotReallySolid[(int)tile2.type]) || TileID.Sets.IsBeam[(int)tile2.type] || WorldGen.IsTreeType((int)tile2.type) && WorldGen.IsTreeType((int)Main.tile[index1 - 1, index2 - 1].type) && WorldGen.IsTreeType((int)Main.tile[index1 - 1, index2 + 1].type)) || (tile3.active() && ((int)tile3.slope() % 2 != 0) && (Main.tileSolid[(int)tile3.type] && !Main.tileNoAttach[(int)tile3.type] && (!Main.tileSolidTop[(int)tile3.type] && !TileID.Sets.NotReallySolid[(int)tile3.type]) || TileID.Sets.IsBeam[(int)tile3.type] || WorldGen.IsTreeType((int)tile3.type) && WorldGen.IsTreeType((int)Main.tile[index1 + 1, index2 - 1].type) && WorldGen.IsTreeType((int)Main.tile[index1 + 1, index2 + 1].type)) || tile4.active() && Main.tileSolid[(int)tile4.type] && !Main.tileNoAttach[(int)tile4.type] && (!Main.tileSolidTop[(int)tile4.type] || TileID.Sets.Platforms[(int)tile4.type]) && (!TileID.Sets.NotReallySolid[(int)tile4.type] && !tile4.halfBrick()))) && tile1.type != (ushort)4)
